...
function ConstruirTablasTac() {
    ...
    tablaGrupoTac.on('key', function(e, datatable, key, cell, originalEvent) {
            ...
            if (113 == key && !noModal) {
                ...
                $('#datatableProcedimientoTacConsulta tbody tr td').eq(0).click();
                tablaTecnologosTac.cell.blur();
    ...
    tablaProcedimientosTac = $('#datatableProcedimientoTacConsulta').DataTable({
        ...
        tablaProcedimientosTac.on('key', function(e, datatable, key, cell, originalEvent) {
            ...
            if (key == 13) {
                ...
                tablaProcedimientosTac.cell.blur();
                $("#datatableProcedimientoTacConsulta_filter input[type=search]").val("");
                $("#datatableProcedimientoTacConsulta_filter input[type=search]").focus();                
            } else {
        ...
        $("#datatableProcedimientoTacConsulta_filter").on('keyup',"input[type=search]", function(e) {            
            if (e.keyCode == 40) {
                $(this).blur();
                $('#datatableProcedimientoTacConsulta tbody tr td').eq(0).click();            
            }        
...
function CargarTodosProcedimientosTacFactura() {
    ...
    $.ajax({
        ...
        $.each(respuesta, function(i, value) {
            ...
            ,respuesta[i][3]];
            tablaProcedimientosTac.row.add(campos).node().id = respuesta[i][0];     
        });
        tablaProcedimientosTac.draw(false);
...
$('#modal-tac').on('shown.bs.modal', function() {
    if(parseInt(ConsultaCargada)>0){
        $("#datatableGrupoTacConsulta").fadeOut(0);
        $("#datatableProcedimientoTacConsulta").fadeOut(0);
    }
    noModal=false;
...
$('#modal-tac').on('hidden.bs.modal', function () {
  try {
    ...
    $('input.checkProceTacFact').prop('checked',false);
         tablaTecnologosTac.clear().draw();
         tablaProcedimientosTac.search("").draw();
    } catch (error) {}
});
