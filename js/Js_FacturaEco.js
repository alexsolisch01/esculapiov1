...
function ConstruirTablasEco() {
    ...
    tablaGrupoEco.on('key', function(e, datatable, key, cell, originalEvent) {
            ...
            if (113 == key && !noModal) {
                ...
                $('#datatableProcedimientoEcoConsulta tbody tr td').eq(0).click();
                tablaTecnologosEco.cell.blur();
     ...
     tablaTecnologosEco.on('key', function(e, datatable, key, cell, originalEvent) {
        ...
        tablaProcedimientosEco.on('key', function(e, datatable, key, cell, originalEvent) {
            ...
            if (key == 13) {
                ...
                tablaProcedimientosEco.cell.blur();
                $("#datatableProcedimientoEcoConsulta_filter input[type=search]").val("");
                $("#datatableProcedimientoEcoConsulta_filter input[type=search]").focus();                              
        ...
        $("#datatableProcedimientoEcoConsulta_filter").on('keyup',"input[type=search]", function(e) {            
            if (e.keyCode == 40) {
                $(this).blur();
                $('#datatableProcedimientoEcoConsulta tbody tr td').eq(0).click();            
            }        
...
function CargarTodosProcedimientosEcoFactura() {
    ...
    $.ajax({
        ...
        $.each(respuesta, function(i, value) {
            ...
            ,respuesta[i][3]];
            tablaProcedimientosEco.row.add(campos).node().id = respuesta[i][0];     
        });
        tablaProcedimientosEco.draw(false);
...
$('#modal-eco').on('shown.bs.modal', function() {
    if(parseInt(ConsultaCargada)>0){
        $("#datatableGrupoEcoConsulta").fadeOut(0);
        $("#datatableProcedimientoEcoConsulta").fadeOut(0);
    }
    noModal=false;
...
$('#modal-eco').on('hidden.bs.modal', function () {
  try {
        ...
        $('input.checkProceEcoFact').prop('checked',false);
         tablaTecnologosEco.clear().draw();
         tablaProcedimientosEco.search("").draw();
    } catch (error) {}
});
