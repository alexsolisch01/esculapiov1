var tablaPacientesEco = null;
var tablapacientesListosEco = null;
...
function ConstruirTablas() {
    ...
    tablaPacientesEco = $('#datatablePacienteEco').DataTable({
        'paging': false,
        'lengthChange': false,
        'searching': true,
        'info': false,
    ...
    tablapacientesListosEco = $('#datatablePacientesListoEco').DataTable({
        keys: true,
        'ordering': false
    });

    tablaProcedimientosEco = $('#datatableProcedimientoPacienteEco').DataTable({
        'paging': false,
        'lengthChange': false,
        'searching': false,
        'info': false,
    ...
    tablaPlantillaEco = $('#datatablePlantillasEco').DataTable({
        'paging': false,
        'lengthChange': false,
        'searching': false,
        'info': false,
...
function CargarPacientesEco() {

    tablaPacientesEco.clear().draw();
    var fechaDesde = $('input#fecha_consultaDesdeEco').val();
    var fechaHasta = $('input#fecha_consultaHastaEco').val();
    ...
    $.ajax({
        ...
        url: "Ajax/Aj_Eco.php",
        data: {
            Requerimiento: "CargarPacientesEco",
        ...
        $.each(pacientes, function (p, itenp) {
            ...
            $.each(respuesta, function (i, iten) {
                if (respuesta[i][0] == pacientes[p]) {
                    ...
                    if (fecha2.toDateString() === hoy.toDateString()) {
                        boton = '<button idConsulta="' + respuesta[i][0] + '" type="submit" id="GuardarRecepcionEco" class="btn btn-success"><i class="fa fa-floppy-o" aria-hidden="true"></i> Guardar</button>';
                    }
            ...
            tablaPacientesEco.row.add(fila).draw(true);
...
function GuardarRecepcionEco(idConsulta) {
    $.ajax({
        async: false,
        method: "POST",
        url: "Ajax/Aj_Eco.php",
        data: {
            Requerimiento: "GuardarRecepcionEco",
            Id: idConsulta
        },
        dataType: 'JSON',
    }).done(function (respuesta) {
...
$(".body").on("click", "button#GuardarRecepcionEco", function (e) {
    ...
    swal({
        ...
        if (confirma) {
            GuardarRecepcionEco($(this).attr('idConsulta'));
        } else {
...
function CargarPacientesListosEco() {


    tablapacientesListosEco.clear().draw();
    var fechaDesde = $('input#fecha_consultaDesdeEcoInforme').val();
    var fechaHasta = $('input#fecha_consultaHastaEcoInforme').val();

    $.ajax({
        method: "POST",
        url: "Ajax/Aj_Eco.php",
        data: {
            Requerimiento: "CargarPacientesListosEco",
            FechaDesde: fechaDesde,
            FechaHasta: fechaHasta
        },
        dataType: 'JSON',
        ...
        $.each(respuesta, function (i, iten) {
            ...
            tablapacientesListosEco.row.add(fila).draw(true);
...
$(".body").on("click", "button#ConsultarReccioncionEco", function (e) {
    CargarPacientesEco();
});

$(".body").on("click", "button#ConsultarPacientesReceptadosEco", function (e) {
    CargarPacientesListosEco();
});

function CargarProcedimientosPacientesEco(idConsulta) {

    var confirma = true;
    tablaProcedimientosEco.clear().draw();

    $.ajax({
        method: "POST",
        url: "Ajax/Aj_Eco.php",
        data: {
            Requerimiento: "CargarProcedimientosPacientesEco",
            Consulta: idConsulta
        },
        dataType: 'JSON',
    }).done(function (respuesta) {
        ...
        $.each(respuesta, function (i, iten) {
            ...
            var campos = ["<span idProceEco=" + respuesta[i][1] + ">" + respuesta[i][2] + "</span>", guardado, validado];
            tablaProcedimientosEco.row.add(campos).draw(false);

        });
        if (confirma) {
            ActualizarEstadoConsulta(idConsultaOrden, "id_estado_orden_eco", idConsulta);
        }
...
$('.body table#datatablePacientesListoEco tbody').on('dblclick', 'tr', function (evt) {
    editor.setValue("");
    ...
    cedula.attr('hcu', fila.find('td').eq(0).find('span').html());
    ...
    CargarProcedimientosPacientesEco(idConsulta);
    CargarMotivoExamen(idOrden);
    ConsultaGuardar = idConsulta;
    PacienteGuardar = paciente;
    fechaAtencion = tablapacientesListosEco.row($(this)).data()[6];
    CargarHistoricoEco(PacienteGuardar)
});
...
function CargarPlantillaEco(idProceEco, nombre) {
    editor.setValue("");
    ProcedimientoGuardar = idProceEco;
    $('button#GuardarResultadoEco').prop('disabled', true);
    $.ajax({
        method: "POST",
        url: "Ajax/Aj_PlantillaEco.php",
        data: {
            Requerimiento: "CargarPlantillaEco2",
        ...
        $('button#GuardarResultadoEco').prop('disabled', false);
...
$('.body table#datatableProcedimientoPacienteEco tbody').on('dblclick', 'tr', function (evt) {
    var IdEco = $(this).find('td').eq(0).find('span').attr('idProceEco');
    var g = $(this).find('td').eq(1).find('i.fa-check-circle-o');
    var v = $(this).find('td').eq(2).find('i.fa-check-circle-o');
    var nombreEco = $(this).find('td').eq(0).find('span').text();
    $('.body').find('label#NombreEco').text(nombreEco);

    if (v.length > 0) {
        CargarResultadoEco(IdEco, ConsultaGuardar);
        $('button#GuardarResultadoEco').prop('disabled', true);
        $('button#ValidarResultadoEco').prop('disabled', true);
        $('button#imprimirDatosEco').prop('disabled', false);
    ...
    if (g.length > 0) {
        CargarResultadoEco(IdEco, ConsultaGuardar);
        $('button#GuardarResultadoEco').prop('disabled', false);
        $('button#ValidarResultadoEco').prop('disabled', false);
        $('button#DesvalidarEco').prop('disabled', true);
        return;
    }

    CargarPlantillaEco1($(this).find('td').eq(0).find('span').attr('idProceEco'));


    $('#modal-plantillas').modal();
});

$('.body table#datatablePlantillasEco tbody').on('dblclick', 'tr', function (evt) {
    var cerrar = $('.body').find('button.close');
    var g = $(this).find('td').eq(1).find('i.fa-check-circle-o');
    var v = $(this).find('td').eq(2).find('i.fa-check-circle-o');
    var NombreEco = tablaPlantillaEco.row($(this)).data()[0];
    var IdEco = tablaPlantillaEco.row($(this)).data()[1];
    if (v.length > 0) {
        CargarResultadoEco(IdEco, ConsultaGuardar);
        $('button#GuardarResultadoEco').prop('disabled', true);
        $('button#ValidarResultadoEco').prop('disabled', true);
    ...
    cerrar.trigger('click');
    CargarPlantillaEco(IdEco, NombreEco);
});

function GuardarResultadoEco(idProceEco, idConsulta, plantilla, idPaciente) {

    $.ajax({
        method: "POST",
        url: "Ajax/Aj_PlantillaEco.php",
        data: {
            Requerimiento: "GuardarResultadoEco",
            Procedimiento: idProceEco,
            Plantilla: plantilla,
            Consulta: idConsulta,
            Paciente: idPaciente,
        ...
        if (respuesta[0] == true) {
            ...
            ActualizarEstadoProcedimientoEcoFactura(idProceEco, idConsulta);
            CargarPacientesListosEco();
            $('button#GuardarResultadoEco').prop('disabled', true);
            return;
        }
...
function ActualizarEstadoProcedimientoEcoFactura(idProceEco, idConsulta) {
    $.ajax({
        method: "POST",
        url: "Ajax/Aj_PlantillaEco.php",
        data: {
            Requerimiento: "ActualizarEstadoProcedimientoEcoFactura",
            Procedimiento: idProceEco,
            Consulta: idConsulta
        },
        dataType: 'JSON',
...
$(".body").on("click", "button#GuardarResultadoEco", function (e) {
    swal({
        title: "Esculapio",
        text: "Seguro Que Desea Guardar?",
        icon: "info",
        ...
        if (confirma) {
            GuardarResultadoEco(ProcedimientoGuardar, ConsultaGuardar, $('textarea#plantillaecoWord').val(), PacienteGuardar);
        } else {
...
function CargarResultadoEco(idProceEco, idConsulta) {
    editor.setValue("");
    ProcedimientoGuardar = idProceEco;
    $('button#GuardarResultadoEco').prop('disabled', true);
    $('button#ValidarResultadoEco').prop('disabled', true);
    $.ajax({
        ...
        url: "Ajax/Aj_PlantillaEco.php",
        data: {
            Requerimiento: "CargarResultadoEco",
            Procedimiento: idProceEco,
            Consulta: idConsulta
        },
        dataType: 'JSON',
...
function CargarPlantillaEco1(idProceEco) {
    $.ajax({
        method: "POST",
        url: "Ajax/Aj_PlantillaEco.php",
        data: {
            Requerimiento: "CargarPlantillaEco1",
            Procedimiento: idProceEco
        },
        dataType: 'JSON',
    }).done(function (respuesta) {
        ...
        tablaPlantillaEco.clear().draw();
        //var idProce1=0;
        $.each(respuesta, function (i, value) {
            //idPaciente1 = respuesta[i][8];
            var dataSet = [respuesta[i][1], respuesta[i][0]];
            tablaPlantillaEco.row.add(dataSet).draw(false);
        });
    }).fail(function (jqXHR, textStatus, errorThrown) {
        console.log(errorThrown);
        //location.reload();
...
function ValidarResultadoEco(idProceEco, idConsulta, plantilla) {

    $.ajax({
        method: "POST",
        url: "Ajax/Aj_PlantillaEco.php",
        data: {
            Requerimiento: "ValidarResultadoEco",
            Procedimiento: idProceEco,
            Plantilla: plantilla,
            Consulta: idConsulta
        },
        ...
        if (respuesta[0] == true) {
            swal("Esculapio!", "Resultados Guardados.!", "success");
            ActualizarEstadoProcedimientoEcoValidadoFactura(idProceEco, idConsulta);
            CargarPacientesListosEco();
            $('button#ValidarResultadoEco').prop('disabled', true);
            $('button#GuardarResultadoEco').prop('disabled', true)
            return;
...
function ActualizarEstadoProcedimientoEcoValidadoFactura(idProceEco, idConsulta) {
    $.ajax({
        method: "POST",
        url: "Ajax/Aj_PlantillaEco.php",
        data: {
            Requerimiento: "ActualizarEstadoProcedimientoEcoValidadoFactura",
            Procedimiento: idProceEco,
            Consulta: idConsulta
        },
        dataType: 'JSON',
...
$(".body").on("click", "button#ValidarResultadoEco", function (e) {
    swal({
        title: "Esculapio",
        text: "Seguro Que Desea Guardar?",
        icon: "info",
        ...
        if (confirma) {
            ValidarResultadoEco(ProcedimientoGuardar, ConsultaGuardar, $('textarea#plantillaecoWord').val());
        } else {
...
$(".body").on("click", "button#imprimirDatosEco", function (e) {
    printTextArea()
});
...
function printTextArea() {
    ...
    var gurdado = CargarReporteDiseno(6);
    ...
    if (gurdado.length > 0) {
        ...
        contenido = contenido.replace("{plantilla}", document.getElementById('plantillaecoWord').value );
...
$(".body").on('click', "button#enviarPorCorreo", function (ev) {
    ...
    var mensaje = document.getElementById('plantillaecoWord').value;
