...
$(".body").on('keyup', "input#ValorTrasferencia", function(evt) {
    CalcularCobro($('#ValorRecibidoConsulta').val(), $('#ValorCheque').val(), $("#ValorTarjeta").val(),$('#totalAnticipo').text().replace('$',''), $("#ValorTrasferencia").val());
    if (evt.keyCode == 13) {
        $('button#CobrarConsultaCobrar').click();
    }
...
function GuardarPagos(idConsulta, valorRecibido, valorRecibidoCheque, valorRecibidoTarjeta, valorCredito, valorRecibidoAnticipo, valorRecibidoTransferencia) {
    if (valorRecibido > 0) {
        var pago = valorRecibido - parseFloat($("span#CambioConsulta").html().replace("$", ""));
        GuardarPagoEfectivo(pago, idConsulta)
    }
    if (valorRecibidoCheque > 0) {
        GuardarPagoCheque(valorRecibidoCheque, idConsulta)
    }
    if (valorRecibidoTransferencia > 0) {
        GuardarPagoTransferencia(valorRecibidoTransferencia, idConsulta)
    }
    if (valorRecibidoTarjeta > 0) {
...
function GuardarPagoTransferencia(valorRecibidoTransferencia, idConsulta) {
    $.ajax({
        method: "POST",
        url: "Ajax/Aj_Forma_pago.php",
        data: {
            Requerimiento: "GuardarPagoTransferencia",
            Consulta: idConsulta,
...
function GuardarPagoAnticipo(valorRecibidoAnticipo, idConsulta, idAnticipo) {
    $.ajax({
        ...
        url: "Ajax/Aj_Forma_pago.php",
        data: {
            Requerimiento: "GuardarPagoAnticipo",
            Consulta: idConsulta,
...
$(".body").on('change', "input#montoT", function(ev) {
    ...
    if (valorCobro <= pendiente) {
        //pendiente = pendiente - valorCobro;
        CalcularCobro($('#ValorRecibidoConsulta').val(), $('#ValorCheque').val(), $("#ValorTarjeta").val(), $('#totalAnticipo').text().replace('$', ''), $("#ValorTrasferencia").val());
        var monto = (isNaN(parseFloat($('#montoT').val()))) ? 0 : parseFloat($('#montoT').val());
        var n = $('#Pagos').val();
        var periodo = $('#cbmPeriodoOdont').val();
        try {
            tablaPagos.clear().draw();
...
function LimpiarCobrar() {
    ...
    $('.body').find('select#bancoTrans').val('0');
