...
function CalcularTotalConsulta() {
    ...
    if(totalcancelar>0){
        $('button#CobrarProformaCobrar').prop('disabled',false);
        $('button#CobrarProforma').prop('disabled',false);
        $('button#CobrarProformaCobrar').fadeIn(0);
        $('button#CobrarProforma').fadeIn(0);
    }else{
        $('button#CobrarProformaCobrar').prop('disabled',true);
        $('button#CobrarProforma').prop('disabled',true);
    }
...
$(".body").on('click', "button#CobrarProforma", function(evt) {
    ...
    swal({
        title: "Esculapio",
        text: "Seguro Que Desea Efectuar esta Proforma ?",
        icon: "info",
        buttons: true,
        dangerMode: false,
    }).then((confirma) => {
...
function GuardarFacturaConsulta() {
    ...
    $.ajax({
        ...
        if (respuesta[0] == true) {
            var edad = calcularEdad($('strong#nombreCompleto').attr("fecha"));
            GuardarDetalleConsulta(respuesta[1], respuesta[2], respuesta[3], edad,respuesta[4]);
...
function GuardarDetalleConsulta(idConsulta, numero, hc, edad,turnos) {
    var vector = $('.body').find("#datatableDetalleFact tbody tr");
    var productos =[];
    var productos_lab =[];
    var productos_rx =[];
    ...
    swal({
        title: "Esculapio",
        text: "Proforma Guardada..!, Desea Imprimir?",
        icon: "info",
        buttons: true,
        dangerMode: false,
    }).then((confirma) => {
...
function LimpiarConsulta() {
    ...
    $('button#CobrarProforma').fadeIn(0);
    $('button#ReimprimirProforma').fadeOut(0);
    ...
    $('button#CobrarProforma').attr("disabled",false);
...
$(".body").on('click', "button#ReimprimirProforma", function(ev) {
   var edad = calcularEdad($('strong#nombreCompleto').attr("fecha"));
...
function ReImprimirTicKetConsulta(idConsulta, numero, hc, edad,emision,autorizacion,productos,productos_lab,productos_rx,productos_eco,productos_tac) {
    ...
    $.ajax({        
        ...
        data: {
            Requerimiento: "ReimprimirProforma",
            Consulta: idConsulta,
...
+"<div style='width:100%; '><label style='font-weight:normal; font-size: 11px;'>PROFORMA #  : "+numero+"</label></div>";
